#+TITLE: Readme
#+SUBTITLE: UFRO-DME-LAB-LAPTOP-CONFIG
#+include: "~/org/config/author-mfuica01.org"
#+AUTHOR: \\
#+AUTHOR: Carlos Jiménez Gallardo
#+DATE: 2022-07-22

* TOC :TOC_1:noexport:
- [[#check-current-cmd-privileges][Check current cmd privileges]]
- [[#install-package-manager][Install package manager]]
- [[#install-git][Install git]]
- [[#install-packages][Install packages]]

* Check current cmd privileges
:PROPERTIES:
:header-args:shell: :tangle batch-files/check-current-cmd-privileges.bat :results silent
:END:

- Source: https://stackoverflow.com/questions/4051883/batch-script-how-to-check-for-admin-rights

#+begin_src shell
@echo off
goto check_Permissions

:check_Permissions
    echo Administrative permissions required. Detecting permissions...

    net session >nul 2>&1
    if %errorLevel% == 0 (
        echo Success: Administrative permissions confirmed.
    ) else (
        echo Failure: Current permissions inadequate.
    )

    pause >nul
#+end_src

* Install package manager
:PROPERTIES:
:header-args:shell: :tangle batch-files/install-chocolatey.bat :results silent
:END:

- =Ubuntu= has =apt= (aka =apt-get=).
- =MacOS= has =brew=.
- =Windows= has =chocolatey=.
  - Source: https://docs.chocolatey.org/en-us/choco/setup

#+begin_src shell
@"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -InputFormat None -ExecutionPolicy Bypass -Command "[System.Net.ServicePointManager]::SecurityProtocol = 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))" && SET "PATH=%PATH%;%ALLUSERSPROFILE%\chocolatey\bin"
pause
#+end_src

* Install git
:PROPERTIES:
:header-args:shell: :tangle batch-files/install-git.bat :results silent
:END:

We use git to keep track of what packages are
supposed to be installed in the machines.

If there is a new package to be installed, we
just update the list, =git fetch= the list and
execute the file.

- Source: https://docs.chocolatey.org/en-us/choco/setup

#+begin_src shell
choco install git
pause
#+end_src

* Install packages
:PROPERTIES:
:header-args:shell: :tangle batch-files/install-packages.bat :results silent
:END:

- Google Chrome
- Firefox
- Libre Office
- Adobe reader
- TexMaker
- VLC
- WinRar
- AIMP
- RStudio
  - Requires to first install =r.project=.
- Geogebra


#+begin_src shell
choco install -y googlechrome firefox libreoffice-fresh adobereader texmaker vlc winrar aimp r.project r.studio geogebra-classic.install
#+end_src
